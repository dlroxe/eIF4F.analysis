# Select TCGA RNA-seq dataset for PCA ------------------------------------------
...
#' @param x number of dimensions kept in the final results
#' @return PCA results
#' @importFrom FactoMineR PCA
#' @examples \dontrun{.RNAseq_PCA(df[[1]], 10)}
#' @keywords internal
#'
...
#' @examples \dontrun{.protein_imputePCA(CPTAC.LUAD.Proteomics.Sample.subset)}
#' @keywords internal
#'
.protein_imputePCA <- function(df) {
...
#' @examples \dontrun{.biplot(res.pca = .RNAseq_PCA(df[[1]], 10), df = df[[2]],
...
#' @importFrom factoextra fviz_contrib fviz_eig fviz_pca_biplot get_pca_var
#' @examples \dontrun{
#' .selected_biplot(res.pca = .RNAseq_PCA(df[[1]], 10), df = df[[2]],
...
plot_PCA_TCGA_GTEX <- function(EIF.list) {
  ...
  .biplot(
    #res.pca = FactoMineR::PCA(df[[1]], # remove column with characters
    #              scale.unit = TRUE,
    #              ncp = 10,
    #              graph = FALSE),
...
plot_PCA_CPTAC_LUAD <- function(EIF.list) {
  ...
  res.pca <- .protein_imputePCA(CPTAC.LUAD.Proteomics.Sample.subset)
